...
const EMPTY_LISTENERS: ListenerEntry[] = [];
...
export class TypedEventEmitter {
  ...
  private _onceListeners: Map<string, ListenerEntry[]> = new Map();
  private _maxListeners: number = 100;
  private _eventNamesCache: string[] | null = null;
  ...
  eventNames(): Array<string | symbol> {
    if (this._eventNamesCache) {
      return this._eventNamesCache as Array<string | symbol>;
    }
  ...
  private _addListener(event: string, listener: EventListener, once: boolean): this {
    // Validate inputs
  ...
  private _removeListener(event: string, listener: EventListener): this {
  ...
  private _handleError(error: unknown): void {
    ...
    if (errorListeners) {
      for (const entry of errorListeners) {
        try {
          (entry.listener as (err: unknown) => void)(error);
        } catch {
   ...
   */
  _getListenerEntries(event: string): { regular: ListenerEntry[]; once: ListenerEntry[] } {
    return {
